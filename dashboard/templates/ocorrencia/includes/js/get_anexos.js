function getAnexos(element, ocorrencia, editable=false) {
    var elemento = $(`#${element}`);
    elemento.children("ul").html("");

    $.ajax({
        url: "{% url 'ocorrencia:get_anexos' %}",
        type: "GET",
        ...
        success: (data) => {
            if(!data["anexos"]){
                ...
                elemento.children("ul").children("li").on("click", "#delete-anexo", function(event){
                    ...
                    deleteelemento($(this), "{% url 'ocorrencia:delete_anexo' %}", `<p>Nenhum anexo inserido.</p>`);
                });
            }
        }
    });
